...
**놓치기 쉬운 노출 지점** (실제로 다 뚫려 있었다 — 카드·히어로만 보고 끝내지 말 것):
- `lib/platforms.ts` 의 `desc` — 헤더 앱 런처와 푸터를 통해 **모든 페이지**에 렌더된다.
- `/quiz` 등 **본문 문구와 `metadata.description`** — 서적 페이지만 훑으면 놓친다.
- **JSON-LD** — 산문은 조심해서 써 놓고 구조화 데이터에 `author: Person` 을 박아두면 기계가 읽는 저작자 주장이 된다. 저자=`Organization`, 사람=`editor` 로 간다(`app/book/**`).
- 반면 토픽 라벨 `"AI · 머신러닝"` 은 **주제**이지 생성 방식이 아니다 — 지우지 말 것.

또한 **없는 기능을 광고하지 않는다**: 서술형·빈칸 채우기 채점은 `ANTHROPIC_API_KEY` 가 있어야 하는데 쓰지 않으므로(daily-quiz 는 객관식만 생성) 공개 문구에서 뺐다. 심사관이 눌러 보고 깨지는 기능은 그 자체로 감점이다.

### AdSense 필수 페이지

`/about`·`/privacy`·`/contact` 는 심사 요건이다. 푸터·`sitemap.ts` 에 등록되어 있어야 한다. `/privacy` 에는 **Google/제3자 광고 쿠키, 맞춤광고 거부 링크, 처리위탁 업체**가 반드시 들어간다. 한국어 서비스이므로 **개인정보 보호책임자**(개인정보 보호법 제31조)도 필수다.

### 🚨 `robots.txt`·`ads.txt` 는 이 zone 이 서빙할 수 없다

`app/robots.ts` 는 basePath 때문에 `/wiki/robots.txt` 로 나가는데, 크롤러는 **오리진 루트**(`logikitapps.com/robots.txt`)만 읽는다. 즉 거기 뭘 쓰든 색인에 영향이 없다:

- NOINDEX 시의 `disallow: "/"` 는 **동작하지 않는다.** 임시 배포의 실질 색인 차단은 `app/layout.tsx` 의 `<meta name="robots" content="noindex">` **하나뿐**이다 — 절대 제거하지 말 것.
- `ads.txt` 도 마찬가지다. **메인 repo 가 `logikitapps.com/ads.txt` 를 소유해야 한다.** AdSense 는 루트만 조회하며, 누락/오기는 표준 경고 사유다.
- 승인 후 sitemap 은 메인 도메인 루트 robots.txt 에 적거나 Search Console 에 직접 제출한다.

### 색인해도 되는 것만 색인한다 (thin content 방지)

빈 페이지를 sitemap 에 넣거나 색인시키면 심사에서 가장 먼저 잡힌다. 서적은 항상 draft 로 들어와 사람이 승인해야 발행되므로 **"토픽·퀴즈는 있는데 보여줄 게 없는" 상태가 정상적으로 존재한다**. 그래서:
- `sitemap.ts` 는 **발행 서적에서 토픽을 역산**한다(`getSitemapData`) — 빈 토픽이 구조적으로 못 들어온다. `getTopics()`(=테이블 전체)로 만들지 말 것.
- 발행 서적 0권인 `/topic/[topic]` 은 `noindex`.
- `/quiz` 는 문항이 있는 토픽만 노출한다(`getQuizCountsByTopic`).
- `/rankings/[window]` 는 `noindex, follow` — 고유 콘텐츠 없는 링크 목록이고 window 3종이 사실상 중복이다. 서적은 이미 sitemap 에 있으므로 색인성 손해도 없다.
...
- **카운터를 세는 테이블/RPC 에 클라이언트 쓰기 권한을 주지 말 것.** 0015 가 막은 두 사고가 전부 이 형태였다: (1) `quiz_attempts` 정책이 `for all` 이라 사용자가 자기 시도를 **DELETE** 해 AI 채점 상한(앱 30회/트리거 60회 — 둘 다 이 테이블의 행 수로 센다)을 리셋할 수 있었다 = 유료 API 무제한. (2) `record_book_view` 가 anon 에게 열린 공개 RPC 라, 번들에 들어 있는 익명 키로 PostgREST 를 직접 때려 랭킹을 조작할 수 있었다. 이제 조회 RPC 는 **service-role 전용**이고 뷰어 해시는 서버에서 만든다(둘은 한 쌍 — 클라가 해시를 고를 수 있으면 중복 제거가 무의미).
- IP 는 `x-real-ip`(Vercel 이 세팅) 우선. `x-forwarded-for` 의 **첫** 항목은 클라이언트가 채워 보낼 수 있어 레이트리밋 우회에 쓰인다.
